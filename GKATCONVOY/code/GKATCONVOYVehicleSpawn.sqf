...
private ["_GKCMoney","_GKCRep","_GKCspawnVehiclePos","_GKCspawnVehicleLoc","_GKCindexVehSpawn","_GKCVehicle","_GKCVehicleArray","_GKCunit"];
...
_GKCVehicleArray = [];
{
	...
	{
	_x setVariable ["GKAT_Unit_Respect", GKCConvoyCrewRep];
	_x addMPEventHandler ["MPKilled",'if (isServer) then { _this call GKATCONVOYRespectPayoutWalker; }; '];
	}forEach (crew _GKCVehicle);
